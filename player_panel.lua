...
function CTtTranslate.show_player_panel(player)
    ...
    local form =
        "formspec_version[3]" ..
        "size[8,6]" ..
        "label[0.3,0.3;CTtTranslate - Translation Panel]" ..

        "dropdown[0.5,1.3;6;player_lang;" ..
        table.concat(lang_names, ",") .. ";" .. dropdown_index .. "]" ..

        "checkbox[0.5,2.6;active;Enable translator;" ..
        (active and "true" or "false") .. "]" ..

        "button_exit[3,4.5;2,1;save;Save]"
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    ...
    local status = pdata.active and "Enabled" or "Disabled"
    minetest.chat_send_player(
        pname,
        "[CTtTranslate] Translator: " .. status
    )
end)