...
function CTtTranslate.show_owner_panel(player)
    ...
    local form =
        ...
        "label[0.6,0.4;CTtTranslate - Owner Control Panel]" ..

        -- ===== GLOBAL =====
        "box[0.3,1.1;23.4,3.0;#222222]" ..
        "label[0.6,1.2;Global Settings]" ..

        "checkbox[1.2,2.1;active; Enable global translator;" ..
        (CTtTranslate.config.active and "true" or "false") .. "]" ..

        "checkbox[1.2,3.0;show_original; Show original language tag;" ..
        (CTtTranslate.config.show_original and "true" or "false") .. "]" ..

        -- ===== API =====
        "box[0.3,4.4;23.4,3.4;#222222]" ..
        "label[0.6,4.5;Translation API]" ..

        "dropdown[1.2,5.4;21.8;api_choice;LibreTranslate,Google Translate;" ..
        (CTtTranslate.config.api == "google" and "2" or "1") .. "]" ..

        "field[1.2,6.6;21.8,1;google_key;Google API Key:;" ..
        (CTtTranslate.config.google_key or "") .. "]" ..

        -- ===== VISUAL =====
        "box[0.3,8.2;23.4,2.2;#222222]" ..
        "label[0.6,8.3;Visual]" ..

        "field[1.2,9.0;10,1;default_color;Default color (HEX);" ..
        (CTtTranslate.config.default_color or "#FF0000") .. "]" ..

        -- ===== TEST =====
        "button[13.5,9.0;9,1;test_translation;Test translation (owner)]" ..

        -- ===== PLAYERS =====
        "label[0.6,10.4;Players (Save permanently)]" ..

        checkboxes ..

        "button_exit[10.5," .. (y + 1.6) .. ";3.5,1;save;Save]"
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    ...
    if fields.test_translation then
        ...
        local tag = "[ES]"
        if CTtTranslate.config.show_original then
            tag = "[ES]"
        end

        local preview =
            minetest.colorize(color, "Hello world (preview)") ..
            " " .. tag
    ...
    minetest.chat_send_player(pname, "[CTtTranslate] Owner settings saved ✅")
end)